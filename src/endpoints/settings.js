...
function backupUserSettings(handle, preventDuplicates) {
    ...
    // Cap to 2 (matches the chats.db backup rotation) instead of the global
    // backups.common.numberOfBackups default of 50, which let settings backups
    // pile up to ~50 copies in the backups folder.
    const maxSettingsBackups = Number(getConfigValue('backups.database.maxBackups', 2, 'number'));
